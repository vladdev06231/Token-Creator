import { Suspense, FunctionComponent, useCallback, useEffect, useState } from 'react';
...
const TransferToken = () => {
    ...
    }, [publicKey, connection, transferAmount]);
    ...
    const getTokenMetadata = useCallback(async (tokenAccount) => {
        ...
        if (metadataAccount) {
            ...
            try {
                ...
                notify({ type: 'error', message: `Failed to fetch token metadata`, description: error?.message });
...
return (
    <div className="my-6">
        {(allTokenList && allTokenList.value) && allTokenList.value.map((value: any, index: number) => {
            ...
            return (
                <div key={"tokenData" + index}>
                    <div
                        ...
                        {logo && <img src={logo} alt="token" width={25} height={25} />}
                        {tokenMetadata &&
                            <div className="flex">
